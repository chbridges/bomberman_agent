...
def setup(self):
    ...
    if reset == 0:
        ...
        global observations, rewards, reg
        observations = np.load('observations.npy')
        rewards = np.load('rewards.npy')
        reward_highscore = rewards[0]
...
def act(self):
    ...
    global epsilon, observations, reg
    ...
    targets.append(look_for_targets(free_space, (x,y), others, self.logger))
    targets.append(look_for_targets(free_space, (x,y), crates, self.logger))
    targets.append(look_for_targets(free_space, (x,y), coins, self.logger))
    targets.append(look_for_targets(free_space, (x,y), bomb_xys, self.logger))
    ...
    # observation = [bomb tag, next other, next crate, next coin, next bomb, next bomb countdown]
    observation = [bombs_left]
    ...
    observation.append(countdown)

    # wait, right, left, up, down, bomb
    actions = ('RIGHT', 'LEFT', 'UP', 'DOWN')
    observation.append(0)
    if np.random.random_sample() < epsilon:
        observation[-1] = np.random.randint(4)
    else:
        max_reward = np.NINF
        max_action = 0
...
def reward_update(self):
    ...
    #rewards[-2] = (1-alpha)*rewards[-2] + alpha*reward
...
def end_of_episode(self):
    ...
    for i in range(-1,1):
        rewards[i-1] = (1-alpha)*rewards[i-1] + alpha*rewards[i]
    if rewards[0] > reward_highscore:
        regr.fit(observations, rewards.ravel())
        np.save('observations.npy', observations)
        np.save('rewards.npy', rewards)
        reward_highscore = rewards[-1]
    observations = np.zeros((0,11))
    rewards = np.zeros((0,1))
